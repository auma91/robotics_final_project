...
def get_nearest_vertex(node_list, q_point):
    ...
    for node in node_list:
        d_temp = np.linalg.norm(node.point-q_point)
        if d_temp < dist:
            closest = node
            dist = d_temp
    return closest
...
def steer(from_point, to_point, delta_q):
    ...
    if dist > delta_q:
        ...
        x = np.sqrt(delta_q**2 / (1 + slope**2))
        new_pt = [from_point[0] + x, from_point[1] + (slope * x)]
        path = np.linspace(from_point, new_pt, num=10, endpoint=True)
    else:
        path = np.linspace(from_point, to_point, num=10, endpoint=True)
    return path
...
def rrt(state_bounds, state_is_valid, starting_point, goal_point, k, delta_q):
    ...
    for i in range(k):
        ...
        near = get_nearest_vertex(node_list, new_point)
        path = steer(near.point, new_point, delta_q)
        validity = check_path_valid(path, state_is_valid)
        if validity:
            n = Node(path[-1], near)
            n.path_from_parent = path
            node_list.append(n)
            if goal_point is not None and np.linalg.norm(n.point - goal_point) < 0.00001:
                return node_list
    return node_list